...
def run_meta_consensus(outputs: list[dict]) -> dict:
    ...
    return {"market": market, "instrument_type": "future", "timeframe": timeframe, "timestamp": timestamp, "agent_key": "futures_meta_consensus", "bias": final_bias, "confidence": round(avg_conf * agreement, 2), "score": round(avg_conf * agreement * 10, 1), "summary": f"Futures consensus is {final_bias} with {round(agreement, 2)} agreement.", "risk_flags": sorted({f for o in outputs for f in o.get("risk_flags", [])}), "features": {"agreement_score": round(agreement, 2), "agent_count": len(outputs)}, "model_version": "futures_meta_consensus_v1"}
